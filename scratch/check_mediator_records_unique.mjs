...
async function run() {
  console.log("Checking unique indexes on 'mediator_records' table...");
  
  const columns = ['mediator_id', 'id'];
  
  for (const col of columns) {
      const { error } = await supabase.from('mediator_records').upsert([{}], { onConflict: col });
