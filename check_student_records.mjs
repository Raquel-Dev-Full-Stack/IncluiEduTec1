...
async function run() {
  const { data, error } = await supabase.from('student_records').select('*').limit(1);
  if (error) {
    console.error("Error fetching student_records:", error);
  } else {
    console.log("STUDENT_RECORDS SCHEMA SAMPLE:");
